...
describe('Take Unit OOS', () => {
  test('the oos manager dialog can be opened', async () => {
    await regionBoardPage.clickUnitCardContextMenuButton('TEST-N001');
  ...
  test('a unit can be taken out of service', async () => {
    await regionBoardPage.confirmUnitStatus('TEST-N001', 'OutOfService');
  });
});

describe('Return unit to service', () => {
  test('the oos manager dialog can be opened', async () => {
    await regionBoardPage.clickUnitCardContextMenuButton('TEST-N001');
    await regionBoardPage.openUnitOosHistory();
  });
  test('a unit OOS event can be ended', async () => {
    await oosManagerPage.endOosEvent();
    await oosManagerPage.closeOosManagerDialog();
  });

  test('a unit can be returned to service', async () => {
    await regionBoardPage.confirmUnitStatus('TEST-N001', 'Available');
  });
});
